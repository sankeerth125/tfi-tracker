import { db } from "./db";
import {
  users, movies, actors, collections,
  type User, type InsertUser,
  type Movie, type InsertMovie,
  type Actor, type InsertActor,
...
export interface IStorage {
  ...
  // Regional
  getRegionalCollectionsByMovieId(movieId: number): Promise<RegionalCollection[]>;
  createRegionalCollection(collection: InsertRegionalCollection): Promise<RegionalCollection>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getRegionalCollectionsByMovieId(movieId: number): Promise<RegionalCollection[]> {
    return await db.select().from(regionalCollections).where(eq(regionalCollections.movieId, movieId));
  }

  async createRegionalCollection(insertCollection: InsertRegionalCollection): Promise<RegionalCollection> {
    const [collection] = await db.insert(regionalCollections).values(insertCollection).returning();
    return collection;
  }
}
...
export class DatabaseStorage implements IStorage {
  ...
  async updateUserProStatus(id: number, isPro: boolean): Promise<User> {
    const [user] = await db.update(users).set({ isPro }).where(eq(users.id, id)).returning();
    return user;
  }
  ...
  async getCollectionsByMovieId(movieId: number): Promise<Collection[]> {
    return await db.select().from(collections).where(eq(collections.movieId, movieId)).orderBy(collections.dayNumber);
  }
