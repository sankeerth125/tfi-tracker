...
export default function Home() {
  ...
  const upcomingMovies = movies?.filter(m => new Date(m.releaseDate) > new Date()) || [];
  const recentMovies = movies?.sort((a, b) => new Date(b.releaseDate).getTime() - new Date(a.releaseDate).getTime()) || [];
      ...
      <main className="container mx-auto px-4 py-8 space-y-12">
          ...
          {runningMovies.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
             ...
             <div className="p-12 text-center border rounded-xl bg-muted/20">
               <p className="text-muted-foreground">No movies currently marked as 'Running'.</p>
             </div>
          )}
        </section>
        ...
        <section className="bg-card border rounded-xl overflow-hidden shadow-sm">
          <div className="p-6 border-b bg-muted/5 flex items-center justify-between">
            <h3 className="font-display font-bold text-xl">DAILY BOX OFFICE TRACKER</h3>
                ...
                {recentMovies.slice(0, 5).map((movie) => (
                  <TableRow key={movie.id} className="hover:bg-muted/5">
                    ...
                    <TableCell className="font-mono">
                      Day {Math.floor((new Date().getTime() - new Date(movie.releaseDate).getTime()) / (1000 * 60 * 60 * 24)) + 1}
                    </TableCell>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {upcomingMovies.length > 0 ? upcomingMovies.map(movie => (
              <div key={movie.id} className="flex items-center gap-4 bg-card p-4 rounded-xl border hover:border-primary/50 transition-colors">
                 <div className="w-16 h-24 bg-muted rounded overflow-hidden flex-shrink-0">
                    <img src={movie.posterUrl} alt={movie.title} className="w-full h-full object-cover" />
                 </div>
                 <div>
                    <h4 className="font-bold font-display text-lg">{movie.title}</h4>
                    <p className="text-sm text-muted-foreground">Release: {new Date(movie.releaseDate).toLocaleDateString()}</p>
                 </div>
              </div>
            )) : (
